...
fps = 50
frame_num = 201
num_steps = int(1 / fps / delta_t)

solver = burgers_solver(delta_x, delta_y, delta_t, mu)

def time_stepping(solution, xs, num_steps):
    solution = solver.solve(solution,  num_steps, show_progress=False, bdry_type='neumann')
    vorticity = get_vorticity(solution, delta_x, delta_y)[:,:,None]
    return solution, jnp.concat([solution, vorticity], axis=-1)
...
vorticity = ys[..., 2]
vorticity = jnp.concat([init_vorticity[None, ...], vorticity], axis=0)

# Convert JAX arrays to numpy for matplotlib
solution_np = np.array(solution)
vorticity_np = np.array(vorticity)
...
im1 = axes[0].imshow(solution_np[0,...,0].T, origin='lower', cmap='plasma', norm=u_norm,
                      extent=[0, x_lim, 0, y_lim], aspect='equal')
axes[0].set_title('u')
axes[0].set_xlabel('x')
axes[0].set_ylabel('y')
...
im3 = axes[2].imshow(vorticity_np[0].T, origin='lower', cmap='plasma', norm=vort_norm,
                      extent=[0, x_lim, 0, y_lim], aspect='equal')
axes[2].set_title(r'$\nabla \times \mathbf{u}$')
axes[2].set_xlabel('x')
axes[2].set_ylabel('y')
...
def animate(frame):
    ...
    im3.set_array(vorticity_np[frame].T)
...
ani.save('animation/neumann_burgers_evolution.mp4', writer='ffmpeg', fps=fps, dpi=200)
