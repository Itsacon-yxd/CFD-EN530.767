...
from burgers_solver import burgers_solver, get_init, get_vortex, penta_solver 
...
init_vortex = get_vortex(init_solution, delta_x, delta_y)
...
def time_stepping(solution, xs, bdry_cond, num_steps, penta_solver):
    solution = solver.solve(solution, bdry_cond, num_steps, penta_solver, show_progress=False)
    vortex = get_vortex(solution, delta_x, delta_y)[:,:,None]
    return solution, jnp.concat([solution, vortex], axis=-1)

time_stepping = jax.jit(
    partial(time_stepping, bdry_cond=bdry_cond, num_steps=num_steps, penta_solver=penta_solver)
)
...
# Initial plots
im1 = axes[0].imshow(solution_np[0,...,0].T, origin='lower', cmap='viridis', norm=u_norm,
...
im2 = axes[1].imshow(solution_np[0,...,1].T, origin='lower', cmap='viridis', norm=v_norm,
                      extent=[0, 2, 0, 1], aspect='equal')
axes[1].set_title('v')
axes[1].set_xlabel('x')
axes[1].set_ylabel('y')
...
im3 = axes[2].imshow(vortex_np[0].T, origin='lower', cmap='RdBu_r', norm=vort_norm,
                      extent=[0, 2, 0, 1], aspect='equal')
axes[2].set_title(r'$\nabla \times \mathbf{u}$')
axes[2].set_xlabel('x')
axes[2].set_ylabel('y')
